...
#define TILE_SIZE 3 // NB // Block SIZE
 ...
 __device__ void store_full(float *g_mem, float *s_mem, int tile_y, int tile_x, int N, int M, int shared_size_single_matrix)
{
    ...
    if(row < N && column < N)
    {
        g_mem[threadIdx.x + row * N * M + column*M] = (tx < TILE_SIZE && ty < TILE_SIZE) ? s_mem[ty * TILE_SIZE + tx + shared_size_single_matrix*threadIdx.x] : 0;
    }
    __syncthreads();
}
 ...
 */
 __device__ void store_lower(float *g_mem, float *s_mem, int tile_y, int tile_x, int N, int M, int shared_size_single_matrix)
 {
    ...
    if(row < N && column < N)
    {
        g_mem[threadIdx.x + row * N * M + column*M] = (tx < TILE_SIZE && ty < TILE_SIZE && column <= row) ? s_mem[ty * TILE_SIZE + tx + shared_size_single_matrix*threadIdx.x] : 0;
    }
    __syncthreads();
 }
 ...
 */
 __device__ void load_full(float *g_mem, float *s_mem, int tile_y, int tile_x, int N, int M, int shared_size_single_matrix)
 {
    ...
    {
        s_mem[tz * TILE_SIZE + ty + shared_size_single_matrix*tx] = (row < N && column < N) ? g_mem[ tx + row * N * M + column*M] : 0; // we need to think about access expression of global memory. //M: Number of matrices. N:dim of matrix
    }
    __syncthreads();
}
 ...
 */
 __device__ void store_zeros(float *g_mem, int tile_y, int tile_x, int N, int M)
 {
    ...
    if(row < N && column < N)
    {
        g_mem[threadIdx.x + row * N * M + column*M] = 0;
    }
    __syncthreads();
 }
...
//MAIN PROGRAM
 ...
 int main() {
    ...
    FILE *fptr;
    fptr = fopen("./Size_4_sample_input.txt", "r");
