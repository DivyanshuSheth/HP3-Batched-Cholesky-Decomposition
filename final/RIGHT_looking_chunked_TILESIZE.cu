...
__device__ void store_full_col(float* read_data,float* write_data,int j,int N, int M, int shared_size_single_matrix)
{
    ...
    for(int i=0;i<N/TILE_SIZE;i++)
    {
        ...
        int x = global_y*N + global_x;	
        int global_id = ((g_threadX / chunk_size) *chunk_size)*N*N + x *chunk_size + (g_threadX % chunk_size);	
 
        write_data[global_id] = read_data[threadIdx.y + TILE_SIZE*global_y + threadIdx.x*shared_size_single_matrix];
    }
    __syncthreads();
    ...
    // {
    //     column = j*blockDim.z + threadIdx.z;
...
__device__ void load_full_col(float* read_data,float* write_data,int j,int N, int M, int shared_size_single_matrix)
{
    ...
    for(int i=0;i<N/TILE_SIZE;i++)
    {
        ...
        write_data[threadIdx.y + TILE_SIZE*global_y + threadIdx.x*shared_size_single_matrix] = read_data[global_id];
...
__device__ void store_full(float* read_data,float* write_data,int i,int j,int N, int M, int shared_size_single_matrix)
{
    ...
    write_data[global_id] = read_data[threadIdx.y + TILE_SIZE*threadIdx.z + threadIdx.x*shared_size_single_matrix];
...
__device__ void load_full(float* read_data,float* write_data,int i,int j,int N, int M, int shared_size_single_matrix)
{
    ...
    write_data[threadIdx.y + TILE_SIZE*threadIdx.z + threadIdx.x*shared_size_single_matrix] = read_data[global_id];
...
__global__ void right_looking_launch_kernel(float* read_data,int N, int M , int num_of_matrices_per_block, int shared_size_single_matrix) // N -> dim, M -> num of matrices per block
{
    ...
    // __shared__ float* tile_data = &row_data[M * (N*TILE_SIZE + TILE_SIZE*TILE_SIZE)];
...
int main()
{
    ...
    // size_t shared_size = (N*TILE_SIZE + TILE_SIZE*TILE_SIZE)*sizeof(float);
    ...
    int N = dim_of_matrix;
    size_t shared_size = num_of_matrices_per_block * (N*TILE_SIZE + TILE_SIZE*TILE_SIZE)*sizeof(float) + num_of_matrices_per_block * TILE_SIZE*TILE_SIZE * sizeof(float);
    
    right_looking_launch_kernel<<<grid,block,shared_size>>>(d_A, dim_of_matrix, num_of_matrices, num_of_matrices_per_block ,(N*TILE_SIZE + TILE_SIZE*TILE_SIZE));
