  ...
  __global__ void left_looking_kernel(float *g_in, int N, int M , int shared_size_single_matrix)
 {
     ...
     for(int i=0; i<no_of_tiles; i++)
     {
         ...
         for(int j=0; j<i; j++)
         {
             ...
             // Rank-k update
             rA1 = s_current_panel + tx*shared_size_single_matrix + no_of_tiles * TILE_SIZE * (TILE_SIZE+1);
             rA2 = s_current_panel + tx*shared_size_single_matrix + i * TILE_SIZE * (TILE_SIZE+1);
             ...
             for(int k=i+1; k<no_of_tiles; k++)
             {
                 ...
                 // sgemm
                 rA1 = s_current_panel + tx*shared_size_single_matrix +no_of_tiles * TILE_SIZE * (TILE_SIZE+1);
                 rA2 = s_current_panel + tx*shared_size_single_matrix +(no_of_tiles + 1) * TILE_SIZE * (TILE_SIZE+1);
                 rA3 = s_current_panel + tx*shared_size_single_matrix +k * TILE_SIZE * (TILE_SIZE+1);
         ...
         rA1 = s_current_panel + tx*shared_size_single_matrix +i * TILE_SIZE * (TILE_SIZE + 1);
         ...
         {
             // trsm
             rA2 = s_current_panel + tx*shared_size_single_matrix +k * TILE_SIZE * (TILE_SIZE +1);
...
__global__ void left_looking_kernel_less_mem(float *g_in, int N, int M , int shared_size_single_matrix)
 {
     ...
     for(int i=0; i<no_of_tiles; i++)
     {
        ...
        for (int j = 0; j < no_of_tiles; j++) {
            rA1 = s_current_panel + j * TILE_SIZE * (TILE_SIZE+1);                  // CHECKKKKK
             ...
             if(j >= i)
             {
                 if(j == i)         // representing the tile on which spotrf will be carried out
                 {
                     for(int k=0; k<i; k++)         // k iterates over tiles left of (i,i) tile
                     {
                         ...
                         __syncthreads();
                         rA2 = s_current_panel + tx*shared_size_single_matrix + 2 * TILE_SIZE * (TILE_SIZE+1); 
                     ...
                     rA1 = s_current_panel + tx*shared_size_single_matrix + 0 * (TILE_SIZE * (TILE_SIZE + 1));
                     spotrf_tile(rA1);
                     __syncthreads();
                     rA1 = &s_current_panel[0];
                 ...
                 else
                 {
                    ...
                    __syncthreads();
                     for(int k=0; k<i; k++)                             // k iterates over tile below (i,i) tile
                     {
                        ...
                        // __syncthreads();
                         rA2 = s_current_panel + tx*shared_size_single_matrix + 3 * TILE_SIZE * (TILE_SIZE+1);
                         load_full(g_in, rA1, j, k, N, M, shared_size_single_matrix);
                        __syncthreads();
                         rA1 = s_current_panel + tx*shared_size_single_matrix + 2 * TILE_SIZE * (TILE_SIZE+1);  // CHECKKK
                         rA3 = s_current_panel + tx*shared_size_single_matrix + 1 * TILE_SIZE * (TILE_SIZE+1);  // CHECKKK
                     ...
                     rA1 = s_current_panel + tx*shared_size_single_matrix + 0 *(TILE_SIZE * (TILE_SIZE+1));
                     rA2 = s_current_panel + tx*shared_size_single_matrix + 1 * TILE_SIZE * (TILE_SIZE +1);
 ...
 int main() {
   ...
   // int blocksPerGrid = (numElements + threadsPerBlock - 1) / threadsPerBlock;
    ...
    if(TILE_SIZE == INPUT_SIZE)
    {
        ...
        left_looking_kernel<<<grid, block, num_of_matrices_per_block * 1 * TILE_SIZE * (TILE_SIZE+1) * sizeof(float)>>>(d_A, dim_of_matrix, num_of_matrices ,1 * TILE_SIZE * TILE_SIZE);
    }
    ...
    else if((no_of_tiles + 2) * TILE_SIZE * (TILE_SIZE+1) * sizeof(float) < devp.sharedMemPerBlock)
    {
        ...
        left_looking_kernel<<<grid, block,num_of_matrices * (no_of_tiles + 2) * TILE_SIZE * (TILE_SIZE+1) * sizeof(float)>>>(d_A, dim_of_matrix, num_of_matrices ,(no_of_tiles + 2) * TILE_SIZE * TILE_SIZE);
    }
    else
    {
        left_looking_kernel_less_mem<<<grid, block, num_of_matrices_per_block * 4 * TILE_SIZE * (TILE_SIZE+1) * sizeof(float)>>>(d_A, dim_of_matrix, num_of_matrices ,4 * TILE_SIZE * TILE_SIZE);
    }
