...
char input_file[] = "InputFiles/num_1024_dim_20.txt";
...
// {
//     int  i = m*TILE_SIZE + threadIdx.z;
//     int  j = n*TILE_SIZE + threadIdx.y;
//     if(!(i<N && j<N))  return;
     
//     arr[threadIdx.z*TILE_SIZE + threadIdx.y] = g_in[i*N + j];
//     __syncthreads(); 
// }
...
//     int  j = n*TILE_SIZE + threadIdx.y;
//     if(!(i<N && j<N)) return;
//     g_in[i*N + j] = arr[threadIdx.z*TILE_SIZE + threadIdx.y];
//     __syncthreads(); 
// }
...
__device__ void load_lower_tile(float *g_mem, float *s_mem, int tile_y, int tile_x, int N, int M, int shared_size_single_matrix)
{
    ...
    //  arr[threadIdx.z*TILE_SIZE + threadIdx.y] = g_in[i*N + j];
    // else arr[threadIdx.z*TILE_SIZE + threadIdx.y] = 0.0;
    // __syncthreads(); 
...
//     {
//         if(threadIdx.y<=threadIdx.z)
//             g_mem[ blockIdx.x * blockDim.x + tx + row * N * M + column*M] = s_mem[tz * TILE_SIZE + ty + shared_size_single_matrix*tx]; // we need to think about access expression of global memory. //M: Total number of matrices. N:dim of matrix
//         else
//             g_mem[ blockIdx.x * blockDim.x + tx + row * N * M + column*M] = 0.0;
//     }
//     __syncthreads();
// }
...
__device__ void store_lower_tile(float *g_mem, float *s_mem, int tile_y, int tile_x, int N, int M, int shared_size_single_matrix)
 ...
 __device__ void store_zeros(float *g_mem, int tile_y, int tile_x, int N, int M)
 {
    ...
    if(row < N && column < N)
    {
        // printf("hello %d %d\n", threadIdx.x ,blockIdx.x * blockDim.x +  threadIdx.x + row * N * M + column*M);
...
int main()
{
    ...
    // NEW
    // cudaMemcpyToSymbol(TILE_SIZE, &TILE, sizeof(TILE));    
    int num_of_matrices_per_block = min(256/(TILE_SIZE * TILE_SIZE) , num_of_matrices);
